...
public class RandomWords {
  ...
  public RandomWords() throws IOException {   
    try (BufferedReader reader = Files.newBufferedReader(Paths.get("words.txt"))) {
      sourceWords = reader.lines().flatMap(line->Stream.of(line.split("\n"))).collect(Collectors.toList());    
  ...
  public List<String> createList(int listSize) {
    ...
    int totalWordsLength = sourceWords.size();
    List<String> wordList = rand
    		.ints()
    		.filter(i ->  i>-1 && i < totalWordsLength)
//    		.distinct()
    		.limit(listSize)
